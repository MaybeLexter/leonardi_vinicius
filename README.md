...
- [13. Cálculo de FPA](#13-cálculo-de-fpa)
    ...
    - [Sistema: Chat com IA](#sistema-chat-com-ia)
    - [Total de Pontos de Função (PF): **81 PF**](#total-de-pontos-de-função-pf-81-pf)
...
| Funcionalidades                          | Qtde | Tipo   | Complexidade | Tamanho    |
|------------------------------------------|------|--------|--------------|------------|
| Entrada de Mensagens (Texto)             | 1    | EE     | Simples      | 3 PF (1x3) |
| Envio de Mensagens (Botão)               | 1    | EE     | Simples      | 3 PF (1x3) |
| Processamento da Mensagem                | 1    | ALI    | Média        | 10 PF (1x10)|
| Resposta da IA                           | 1    | SE     | Simples      | 4 PF (1x4) |
| Exibição do Saldo de Tokens              | 1    | SE     | Média        | 5 PF (1x5) |
| Consulta de Saldo de Tokens              | 1    | CE     | Simples      | 3 PF (1x3) |
| Acesso ao Arquivo Lógico Interno (Histórico) | 1 | ALI  | Média        | 10 PF (1x10)|
| Consumo de API Externa (Sistema de Tokens) | 1   | AIE    | Média        | 7 PF (1x7)|
| Validação do Usuário                     | 1    | EE     | Média        | 4 PF (1x4) |
| Gerenciamento de Sessão                  | 1    | ALI    | Simples      | 7 PF (1x7) |
| Envio de Arquivo (Anexo)                 | 1    | EE     | Média        | 4 PF (1x4) |
| Armazenamento do Arquivo                 | 1    | ALI    | Simples      | 7 PF (1x7) |
| Consulta de Arquivo Anexado              | 1    | CE     | Simples      | 3 PF (1x3) |
| Download de Arquivo Anexado              | 1    | SE     | Simples      | 4 PF (1x4) |

---

### Total de Pontos de Função (PF): **81 PF**

#### Explicação:

- **Entrada de Mensagens (Texto)**: O usuário envia uma mensagem para a IA processar. Considerado uma Entrada Externa (EE) simples.
- **Envio de Mensagens (Botão)**: Quando o usuário clica em "Enviar", a mensagem é processada. Também considerado uma Entrada Externa simples.
- **Processamento da Mensagem**: A IA processa a mensagem para gerar uma resposta, considerado como um Arquivo Lógico Interno (ALI) de complexidade média, pois envolve lógica.
- **Resposta da IA**: A saída da IA, ou seja, a resposta gerada para o usuário, é uma Saída Externa (SE) de complexidade simples.
- **Exibição do Saldo de Tokens**: A IA exibe o saldo de tokens disponível ao usuário. Uma Saída Externa de complexidade média, pois há uma interação com a API para obter o valor.
- **Consulta de Saldo de Tokens**: O usuário pode consultar seu saldo, considerado uma Consulta Externa (CE) de complexidade simples.
- **Acesso ao Histórico (ALI)**: O sistema acessa dados do histórico de mensagens do usuário, Arquivo Lógico Interno de complexidade média.
- **Consumo de API Externa (AIE)**: O sistema se comunica com um serviço externo para verificar o saldo de tokens, Arquivo de Interface Externa (AIE) de complexidade média.
- **Validação do Usuário**: O sistema valida a identidade do usuário para iniciar a sessão, uma Entrada Externa de complexidade média.
- **Gerenciamento de Sessão (ALI)**: O sistema gerencia as sessões ativas do usuário, Arquivo Lógico Interno de complexidade simples.
- **Envio de Arquivo (Anexo)**: O usuário pode anexar arquivos, uma Entrada Externa de complexidade média, devido às validações de arquivo (tamanho, formato).
- **Armazenamento do Arquivo (ALI)**: O sistema armazena o arquivo anexado, considerado um Arquivo Lógico Interno de complexidade simples.
- **Consulta de Arquivo Anexado (CE)**: O usuário pode visualizar ou consultar os arquivos anexados, uma Consulta Externa de complexidade simples.
- **Download de Arquivo Anexado (SE)**: O usuário pode fazer download do arquivo anexado, uma Saída Externa de complexidade simples.
